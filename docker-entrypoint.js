...
;(async() => {
  ...
  if (command === 'pnpm run start') {
    ...
    // Use next start from node_modules
    // For standalone mode, next start reads from .next/standalone
    await exec('node /app/node_modules/.bin/next start', env)
  } else {
...
function exec(command, env) {
  ...
  const child = spawn(command, { shell: true, stdio: 'inherit', env, cwd: '/app' })
